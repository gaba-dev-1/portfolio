...
function RingNodes({ 
  ...
  const nodeMaterial = useMemo(() => {
    return new THREE.MeshPhysicalMaterial({
      ...
      envMapIntensity: isDark ? 0.8 : 0.6
    })
  }, [mainColor, isDark])
...
function Ring({ 
  ...
  const emissiveColor = new THREE.Color(color).multiplyScalar(isDark ? 1.0 : 0.8)
  
  const ringMaterial = useMemo(() => {
    return new THREE.MeshPhysicalMaterial({
      color: ringColor,
      emissive: emissiveColor,
      emissiveIntensity: isDark ? 0.5 : 0.4,
      metalness: 0.8,
      roughness: 0.15,
      transparent: true,
      ...
      transmission: 0.1,
      envMapIntensity: isDark ? 0.6 : 0.5
    })
...
function GlassCore({ 
  ...
  const innerColor = new THREE.Color(color).multiplyScalar(isDark ? 1.5 : 1.3)
  ...
  const sphereMaterial = useMemo(() => {
    return new THREE.MeshPhysicalMaterial({
      ...
      ior: 1.8,
      thickness: 1.5,
      envMapIntensity: isDark ? 1.5 : 1.2,
    })
  }, [coreColor, isDark])
...
export default function SceneController({ speed = 1 }: { speed?: number }) {
  ...
  const memoizedScene = useMemo(() => (
    ...
    >
      ...
      <ambientLight intensity={isDark ? 0.3 : 0.25} />
      
      {/* Main point light with better visibility */}
      <pointLight 
        position={[0, 0, 0]} 
      ...
      {/* Additional rim lighting for better definition */}
      <directionalLight
        position={[5, 5, 5]}
      ...
      <Suspense fallback={null}>
        ...
        {/* Subtle bloom for a nice glow without being overwhelming */}
        <EffectComposer>
          <Bloom 
